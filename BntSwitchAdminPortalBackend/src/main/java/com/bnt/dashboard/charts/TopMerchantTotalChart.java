...
public class TopMerchantTotalChart implements Charts<KeyPair> {

	public DashBoardData<KeyPair> buildContent(ChartsConfig chartsConfig) {
		Content<KeyPair> content = DashboardStats.buildContent(Lists.newArrayList(DashboardStats.Trnsactions.MERCHANT),
				chartsConfig);
		...
		return new DashBoardData<>(BLOCK_TYPE_HALF, TOP_MERCHANT_COUNT_DONUT_TITLE, ACTION_LINK, ACTION_TITLE, content);
	}
}

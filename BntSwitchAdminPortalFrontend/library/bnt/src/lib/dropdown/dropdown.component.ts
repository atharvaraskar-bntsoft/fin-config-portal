...
@Component({
  ...
  selector: 'rv-dropdown-toggle',
  template: '<ng-template #templateRef><ng-content></ng-content></ng-template>',
})
export class DropdownToggleComponent {
  @ViewChild('templateRef', { static: true }) public templateRef: TemplateRef<any>;
  @ContentChild('toggleElement', { static: true })
...
@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,
  selector: 'rv-dropdown, [rv-dropdown]',
...
export class DropdownComponent implements AfterViewInit, OnDestroy {
  ...
  @ContentChild(DropdownToggleComponent, { static: true })
  public dropdownToggleComponent: DropdownToggleComponent;
  @ContentChild(DropdownMenuComponent, { static: true })
  public dropdownMenuComponent: DropdownMenuComponent;

  @ViewChild('toggleElement', { static: true })
   ...
   */
  ngAfterViewInit() {
    const toggleNativeElement =
      this.dropdownToggleComponent && this.dropdownToggleComponent.toggleElement
        ? this.dropdownToggleComponent.toggleElement.nativeElement
        : this.toggleElement
        ? this.toggleElement
        : this.defaultToggleElement
        ? this.defaultToggleElement.nativeElement
        : null;
    if (toggleNativeElement) {
      this.ngZone.runOutsideAngular(() => {
        this.listeners.push(
          this.renderer2.listen(toggleNativeElement, 'click', (event: Event) => {

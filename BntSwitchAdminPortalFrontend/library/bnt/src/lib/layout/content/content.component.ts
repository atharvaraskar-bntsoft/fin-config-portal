...
export class ContentComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
      ...
      this.routingService.onChange.subscribe((value: any) => {
        if (value && value[value.length - 1]) {
          ...
          this.translate.get([title]).subscribe(translation => {
            const getTitle = this.getTitle(translation[title]);
            this.titleService.setTitle(getTitle);
            this.header = getTitle;
    ...
    this.heightStyle = this.windowInnerHeight - 101;
    // this.heightStyle = 85;
  }
  ...
  private setContentMinHeight(): void {
    if (this.navigationEnd) {
      ...
      const headerFooterOffsetHeight =
        this.headerService.offsetHeight + this.footerService.offsetHeight;
      ...
      if (this.layout === 'fixed') {
        ...
        const sidebarRight = this.sidebarRightService.scrollHeight
          ? this.sidebarRightService.scrollHeight - this.headerService.offsetHeight
          : 0;

...
export class TabComponent implements OnInit {
  ...
  @ContentChild(TabHeaderComponent, { static: true })
...
export class TabsComponent implements AfterContentInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this.subscriptions.push(
      this.tabToggleDirectives.changes.subscribe(() => {
        this.setTabsToggle();
      }),
    );
  }
  ...
  public openTabIndex(): void {
    if (this.tabs) {
      this.tabs.forEach((tab: TabComponent) => {
        if (
          this.activatedTabIndex === tab.index ||
          (this.activatedTabIndex === undefined && tab.index === 0)
        ) {
          tab.isActive = true;
  ...
  private setTabsToggle(): void {
    ...
    this.ngZone.runOutsideAngular(() => {
      this.tabToggleDirectives.forEach((tabToggle: TabToggleDirective) => {
        this.listeners.push(
          this.renderer2.listen(tabToggle.elementRef.nativeElement, 'click', event => {

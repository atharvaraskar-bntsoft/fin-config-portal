...
import { TSaveMethods } from '@app/imf-json/imf-json-create/imf-json-create.component';
...
export class ImfHierarchyTreeComponent implements OnInit, OnDestroy {
      ...
      if (idx === index.length - 2) {
        ...
        switch (payload.type) {
          case 'fields':
            ...
            // el = fields.find(ele => (ele.name === this.currentSelectedNode.name || ele.fieldName === this.currentSelectedNode.fieldName) )
            break;
          default:
            // el = obj.attributes.find(
            //   (elment) =>
            //     elment.name === payload.fieldName ||
            //     elment.fieldName === payload.fieldName
            // );
            el = obj.attributes
              .filter(
                ele =>
  ...
  removeNode(item) {
    ...
    if(this.initialJson.details.mapping){
        let mapping = JSON.parse(JSON.stringify(this.initialJson.details.mapping));
         findsources = mapping.sources.find(({ source }) => source === item.name);
       }
    if(!findsources){
    this.isItemForEdit = false;
    ...
    this.treeControl.expand(this.treeControl.dataNodes[0]);
        ...
        this.alertService.responseMessage({
            status: 'failure',
            message: 'this field can not delete',
          });
    }
   // this.expandNode();
  }

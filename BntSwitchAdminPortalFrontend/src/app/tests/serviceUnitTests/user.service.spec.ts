...
describe('userservice', () => {
    ...
    it('should retrieve getUsers', () => {
        userService.getUsers(limit => {
            expect(limit).toBeTruthy();
            const req = httpTestingController.expectOne(`${basePath.domain}${UsersUrls.getUsers}`);
            expect(req.request.method).toEqual('GET');
        req.flush({velocityLimitJson});
        });
    });
    it('should retrieve getRoleList', () => {
        userService.getRoleList(limit => {
            expect(limit).toBeTruthy();
            const req = httpTestingController.expectOne(`${basePath.domain}${UsersUrls.getRoles}`); //TODO :: extract URL to const and reuse in spev and service
            expect(req.request.method).toEqual('GET');
            req.flush({velocityLimitJson}); 
        });
    });
    it('should retrieve getUsersDetails', () => {
        userService.getUsersDetails(limit => {
            expect(limit).toBeTruthy();
            const req = httpTestingController.expectOne(`${basePath.domain}${UsersUrls.getUsers}`); //TODO :: extract URL to const and reuse in spev and service
            expect(req.request.method).toEqual('GET');
            req.flush({velocityLimitJson}); 
        });
    });
    ...
    it('should retrieve deleteUsers', () => {
        let id = 1;
        userService.deleteUsers(limit => {
            expect(limit).toBeTruthy();
            const req = httpTestingController.expectOne(`${basePath.domain}${UsersUrls.getUsers}${id}`); //TODO :: extract URL to const and reuse in spev and service
            expect(req.request.method).toEqual('DELETE');
            req.flush({velocityLimitJson});
        });
    });
    it('should retrieve postUsers', () => {
        let id = 1;
        userService.postUsers(corePrperties => {
          expect(corePrperties).toBeTruthy();
          const req = httpTestingController.expectOne(`${basePath.domain}${UsersUrls.getUsers}${id}`);
          expect(req.request.method).toEqual('POST');
          req.flush({});
        });
      });
      // it('should retrieve putUsers', () => {
      //   let id = 1;
      //   userService.putUsers(corePrperties => {
      //     expect(corePrperties).toBeTruthy();
      //     const req = httpTestingController.expectOne(`${basePath.domain}${UsersUrls.getUsers}${id}`);
    ...
    //     });
    // });
    // it('should retrieve putUsersStatus', () => {
    //     let id = 1;
    //     userService.putUsersStatus(limit => {
    //         expect(limit).toBeTruthy();
    //         const req = httpTestingController.expectOne(`${basePath.domain}${UsersUrls.getUsers}${id}`);
    //         expect(req.request.method).toEqual('PUT');
    //         req.flush({velocityLimitJson});

...
@Component({
  selector: 'rule-condition-node',
  templateUrl: './rule-condition-node.component.html',
  styleUrls: ['./rule-condition-node.component.scss'],
})
export class RuleConditionNodeComponent implements OnInit {
  ...
  public createGroup(rules: any, id: number) {
    // id = JSON.parse(JSON.stringify(rules.id));
    rules.id = id + '-' + this.nodes[id].nodes.length;
    this.nodes[id] = { delete: false, group: true, nodes: [], relation: '', id: id };
    this.nodes[id].nodes.push(rules);
    // this.nodes[id].nodes.push({
    //     delete: false,
    //     group: false,
    //     key: null,
    //     service: null,
    ...
    //     message: null,
    //     value: null,
    //     id: id + '-' + this.nodes[id].nodes.length,
    // });
  }
  ...
  public typeChange(e?: any, node?: any) {
    if (e.name === 'Request' && node.key.indexOf('.request_message') > -1) {
      node.key = node.key.replace('response_message', 'request_message');
      node.message = 'request_message';
    } else if (node.key.indexOf('.response_message') > -1 && e.name === 'Response') {
      node.key = node.key.replace('request_message', 'response_message');
  ...
  isValid(str) {
    return /^([a-zA-Z0-9\s]+,)*([a-zA-Z0-9\s]+){1}$/g.test(str);

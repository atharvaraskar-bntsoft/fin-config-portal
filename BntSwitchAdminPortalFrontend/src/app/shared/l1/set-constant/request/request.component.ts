...
@Component({
  selector: 'l1-set-constant-request',
  templateUrl: './request.component.html',
  styleUrls: ['./request.component.scss'],
})
export class L1SetConstantRequestComponent implements OnInit {
  @Input() mappings;
  ...
  @Input() enrichfields;
  @Input() isDataEnrichmentScreenVisible;
  ...
  public pushConstItem() {
    this.mappings.push({ type: 'field', source: null, destination: null, condition : null });
  }
    ...
    if (this.enrichfields === 'configuration') {
      ...
      if (this.enrichModelObject) {
        ...
        this.enrichModelObject.fields.map(field => {
          if (!field.destination || !field.source.trim()) {
            this.dataEnrichmentScreenError = true;
          } else {
            this.dataEnrichmentScreenError = false;
          }

...
@Component({
  selector: 'l1-response-mapper',
  templateUrl: './response.component.html',
  styleUrls: ['./response.component.scss'],
})
export class L1ResponseMapperComponent implements OnInit {
  @Input() mapperList = [];
  @Input() field;
  @Input() sourceList;
  @Input() imfList;
  ...
  public selectSourceParam(event, param) {
    ...
    if (event.id.indexOf('http_headers[') > -1) {
      ...
      param.transformValue =
        '${message_exchange[GATEWAY_SERVICE].native_response_message[' + event.id + ']}';
    }
    param.error = false;
  }

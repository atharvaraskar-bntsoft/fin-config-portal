...
@Component({
  selector: 'app-responsetab-http-json',
  templateUrl: './responsetab-http-json.component.html',
  styleUrls: ['./responsetab-http-json.component.scss']
})
export class ResponsetabHttpJsonComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    if (this.adapterData) {
      ...
      if (this.isEdit && this.responseCodeData.ipcUiWrapper) {
        ...
        this.defaultResponseDesc = this.adapterData.responseCodeData.ipcUiWrapper.defaultResponseDesc;
        this.apiList = [];
        if(this.adapterData.responseCodeData.ipcUiWrapper.componentResponseCodeField){
        this.strToArray(this.adapterData.responseCodeData.ipcUiWrapper.componentResponseCodeField).forEach(elementouter => {
  ...
  usedAPIChange(apilist) {
    ...
    this.apiList.forEach(element => {
      this.internalAPIList.forEach((elementapi, index) => {
        if (element.api === elementapi.name) {
          this.internalAPIList.splice(index, 1);
        }
      });
    });
  }
  ...
  public draftResponse(flag = true) {
    ...
    if (!newAlphaNumeric.test(this.responseCode) && this.template.value.indexOf('SOAP') === -1 && isValid) {
      this.alertService.responseMessage({
        status: 'failure',
    ...
    if (isValid) {
      if (this.adapterData.responseCodeData.ipcUiWrapper) {
        ...
        this.adapterData.responseCodeData.ipcUiWrapper.defaultResponseDesc = this.defaultResponseDesc;
        if (this.apiList.length !== 0) {
          this.adapterData.responseCodeData.ipcUiWrapper.componentResponseCodeField = this.jsonArrayToString(this.apiList);
        }
  ...
  pushSimpleObject(finalObject, apiName, responseName, responseCode, responseDesc) {
    var newStrObj = {
      ...
      "responseCode": (responseCode)? responseCode.replace('.', '&#46;'): '',
      "responseDesc": (responseDesc) ? responseDesc.replace('.', '&#46;'): ''
    };

...
@Component({
  ...
  styleUrls: ['./transform-json.component.scss'],
  templateUrl: './transform-json.component.html',
})
export class TransformJsonComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.adapterData.transformData.apiFieldsData) {
      ...
      if (this.jsonTransactions.length) {
        ...
        this.getAuthReqData(0, this.jsonTransactions);
        this.getAuthResData(0, this.jsonTransactions);
  ...
  public apiRequestResponse(event) {
    ...
    this.selectedTransactionType = event.transactionTypeValueList.length
      ...
      : null;
    if (this.authReqDivShow == 'req' && event) {
      this.authData = event?.incomingFields;
    ...
    } else if (this.authReqDivShow == 'res' && event) {
      this.authData = event?.outGoingFields;
      this.adapterData.transformData.listIdRule = event?.outGoingFields.concat(
        this.adapterData.transformData.apiFieldsData.headerFields,
      );
    }
    this.getPreActionData();
    ...
    this.tabChange(this.selectedTransactionType + ' Request', 0);
  }
  ...
  public jsonSchemaRequestResponse() {
    ...
    if (!output) {
      this.jsonCondition = null;
      this.adapterData.transformData.listIdRule = this.apiName?.incomingFields.concat(
        this.adapterData.transformData.apiFieldsData.headerFields,
      );
      this.jsonTransactions.push({
  ...
  public showTransactionCondModal() {
    ...
    if (result && result.condition) {
      this.conditionObj = JSON.parse(JSON.stringify(result.condition));
    }
    this.jsonTransactions.map(itemmap => {
      if (itemmap.messageIntentifier !== this.selectedTransactionType) {
        this.transactionRequestArray.push(itemmap);
        this.transactionResponseArray.push(itemmap);
      }
    });
  ...
  public tabChange(tab, i) {
    ...
    if (tab === this.selectedTransactionType + ' Response') {
      ...
      this.getAuthResData(index, this.jsonTransactions);
      this.apiRequestResponse2(this.apiName);
    } else if (tab === this.selectedTransactionType + ' Request') {
      this.authReqDivShow = 'req';
      this.selectedTabIndex = i;
      this.getAuthReqData(index, this.jsonTransactions);
      this.apiRequestResponse2(this.apiName);
  ...
  public delAuthResField(data) {
    ...
    this.jsonTransactions.map(item1 => {
      if (
        ...
        item1.response.mappings = item1.response.mappings.filter(x => x && x.source !== source);
      }
      return item1;
    });
  ...
  public transactionSelection(value) {
    if (value && value.nzValue) {
      ...
      if (reqTransaction > -1) {
        ...
        this.getAuthReqData(reqTransaction, this.jsonTransactions);
      } else if (resTransaction > -1) {
        this.transformActionItemRes = [];
        this.getAuthResData(resTransaction, this.jsonTransactions);
      } else {
      ...
      if (event.selectedOption === 'operationField' || event.selectedOption === 'copyField') {
        ...
        if (value.source && value.destination) {
          ...
          if (value.destination[0].indexOf('http_headers[') > -1) {
            ...
            source = value.destination[0].split('native_request_message[')[1].split(']')[0];
          }
  ...
  private setIntialSchemaArrayMapper(fieldId, value) {
    ...
    console.log('loopArray', this.loopArray);
  ...
  findGetSetArrayMapper(fieldId, value?) {
    ...
    if (output) {
      if (!this.isRequest) {
        this.loopArray = [];
        const requestMapping = output.response.mappings.find(
        ...
        output.response.mappings = output.response.mappings.filter(
          el => el.field !== '${' + fieldId.parentField.split('|')[0] + '}',
        );
        if (mapping) {
          output.response.mappings.push(JSON.parse(JSON.stringify(mapping)));
        }
      }
    }
  }
  ...
  public copyReqMapFn() {
    this.jsonTransactions.forEach((transactions, transactionsId) => {
      if (transactions.messageIntentifier === this.selectedTransactionType) {
        ...
        this.getReqMapping(this.jsonTransactions, transactionsId);
      }
    });
  }
  public copyResMapFn() {
    this.jsonTransactions.forEach((transactions, transactionsId) => {
      if (transactions.messageIntentifier === this.selectedTransactionType) {
        ...
        this.getAuthResData(transactionsId, this.jsonTransactions);
      }
    });
  }
        ...
        if (
          ...
          item.request.mappings = item.request.mappings.filter(
            mapping =>
              ...
              (mapping.type !== 'field' || mapping.ipc),
          );
        }
        return item;
      });
  ...
  public selectWhereField(element) {
      ...
      if (
        ...
        this.l3ReqConstant.forEach(item => {
          if (item.source && item.destination) {
            ...
            if (item.singleDestination) {
              item.destination = destination[0];
            } else if (!item.singleDestination && !item.resImfValue) {
              destination[1] =
                '${message_exchange[' +
                this.networkService +
                '].additional_fields[' +
            ...
            if (item.useCase == 1 && !item.singleDestination && item.resImfValue) {
              destination[1] = '${' + item.resImfValue + '}';
            } else if (item.useCase == 2 && !item.singleDestination && item.resImfValue) {
              destination[1] =
            ...
            if (
              ...
              if (it.fieldId) {
                if (source !== it.fieldId) {
                  this.responseImfField[source + this.selectedTransactionType] = it.name;
                  if (!this.transformActionItemRes[source + this.selectedTransactionType]) {
                    this.transformActionItemRes[source + this.selectedTransactionType] = [];
                  }
                  this.selectSourceResData[source + this.selectedTransactionType] = true;
                  this.transformActionItemRes[source + this.selectedTransactionType].push(it);
                } else {
                  source = it.fieldId;
                  this.responseImfField[source + this.selectedTransactionType] = it.name;
                }

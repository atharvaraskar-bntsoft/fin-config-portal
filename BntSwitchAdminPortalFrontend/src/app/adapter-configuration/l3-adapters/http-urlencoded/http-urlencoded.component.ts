  ...
  */
  ...
  public openRequestPayload(i) {
    ...
    if (
      ...
      this.setInvalidField(this.jsonData[this.currentIndex]);
  ...
  setInvalidField(item) {
    item?.attributes?.forEach(element => {
      if (element.type === 'field') {
        let fieldName = JSON.parse(JSON.stringify(element.fieldName));
        const name = this.renameString(fieldName);
      }
    });
  }
  ...
  public openResponsePayload(i) {
    ...
    if (
      ...
      this.setInvalidField(this.resJsonData[this.currentIndex]);

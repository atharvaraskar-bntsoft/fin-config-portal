...
@Component({
  selector: 'app-l3-xml-over-http',
  templateUrl: './l3-xml-over-http.component.html',
  styleUrls: ['./l3-xml-over-http.component.scss'],
})
export class L3XmlOverHttpComponent implements OnInit, OnDestroy {
  ...
  public xmlOverHttp: boolean = true;
  public httpreg =
  ...
  */
  ...
  getHeadersFormGroup(index): UntypedFormGroup {
    return this.headersList.controls[index] as UntypedFormGroup;
  }

  getapiFormGroup(index): UntypedFormGroup {
    return this.apisList.controls[index] as UntypedFormGroup;
  }
  ...
  public closeHeaderPopup() {
    ...
    for (let i = this.headersList.length; i >= 0; i--) {
  ...
  public saveHeaderPopup() {
    ...
    this.form.value.headers.forEach((item, _index) => {
  ...
  logobject(p) {
    ...
    for (let key in p) {
      if (key && typeof p[key] == 'string') {
        ...
        let childobject = this.processObject(key, p[key]);
        finalJsonObject.attributes.push(childobject);
      } else if (key && typeof p[key] == 'object' && Array.isArray(p[key])) {
        let nestedList = this.processList(key, p[key]);
  ...
  public requestTransform(): void {
    ...
    if (this.reqData[this.currentIndex]) {
      try {
        this._l1AdapterService
          .transformXmlpackager(this.reqData[this.currentIndex])
          .subscribe(item => {
            if (item) {
              this.requestActiveTab = 1;
              let currentReq = JSON.parse(item.data.responseData);
              this.jsonData[this.currentIndex] = currentReq[0];
  ...
  public responseTransform(): void {
    this.inValidFieldList = [];
    if (this.resData[this.currentIndex]) {
      try {
        this._l1AdapterService
          .transformXmlpackager(this.resData[this.currentIndex])
          .subscribe(item => {
            this.resJsonData[this.currentIndex] = JSON.parse(item.data.responseData);
            this.responseActiveTab = 1;
            let currentReq = JSON.parse(item.data.responseData);
            this.resJsonData[this.currentIndex] = currentReq[0];
  ...
  public checkdefaultHost() {
    ...
    if (!this.form.value.defaultHost) {
      this.form.value.apiDefinitions.forEach(element => {
        if (element.api && element.name && element.method) {
          flag = true;
        } else {
          flag = false;
        }

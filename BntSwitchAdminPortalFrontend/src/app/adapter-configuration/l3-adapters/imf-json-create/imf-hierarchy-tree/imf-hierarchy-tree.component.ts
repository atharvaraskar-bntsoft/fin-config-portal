...
export class ImfHierarchyTreeComponent implements OnInit, OnDestroy {
      ...
      if (idx === index.length - 2) {
        if (obj.attributes[index[index.length - 1]]) {
          for (let item of obj.attributes) {
            if (
              item.name === this.currentSelectedNode.name ||
              item.fieldName === this.currentSelectedNode.fieldName
            ) {
              if (item['xmlAttributes'] && item['xmlAttributes'].length > 0) {
                ...
                let el = item['xmlAttributes'].find(
                  ele => ele.name === newIMFNode.name || ele.fieldName === newIMFNode.fieldName,
                );
                if (el) {
                  if (actions === 2) {
                    let deleteIndex = item['xmlAttributes'].findIndex(
                      e => e.fieldName === newIMFNode.fieldName,
                    );
  ...
  addXmlAttribute(item) {
    this.isItemForEdit = false;
    item.nodeAdded = true;
    this.subscribeService.updateSelectedIMF({
      type: 'field',
      xmlType: 'xmlHttp',
      fieldType: 'STRING',
    });

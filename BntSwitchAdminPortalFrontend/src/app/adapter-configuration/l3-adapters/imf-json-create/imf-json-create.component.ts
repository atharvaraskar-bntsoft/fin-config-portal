...
export class ImfJsonCreateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscription.add(
      this.subscribeService.getItems().subscribe(obj => {
        this.jsonData = this.jsonData;
      }),
    );
    this._store.pipe(takeUntil(this.componetDestroyed)).subscribe(res => {
      this.alertService.getIsBtnDisable().subscribe(res => {
        // console.log('Res', res);
        if (res) {
          this.isSaveEnable = res.isEnable;
        }
      });
    });
  ...
  tabViewChange(event) {
    if (event.index === 1) {
      if (this.jsonData) {
        this._store.dispatch(new UserViewJson(this.jsonData));
      }
    }
  }
  ...
  public version(): void {
    ...
    // this.currentItem.imf = JSON.stringify(this.jsonData);
    // this._store.dispatch(new CreateImfJson(this.currentItem));
  }
  ...
  public save(): void {
    // this.currentItem.imf = JSON.stringify(this.jsonData);
    // this._store.dispatch(new UpdateImfJson(this.currentItem));
    const currentItem = { ...this.currentItem };
    currentItem.imf = {
      attributes: {},
    };

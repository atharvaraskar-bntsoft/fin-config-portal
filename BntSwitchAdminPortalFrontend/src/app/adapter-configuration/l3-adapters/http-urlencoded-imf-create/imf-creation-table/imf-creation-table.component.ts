...
@Component({
  selector: 'app-urlencoded-imf-creation-table',
  templateUrl: './imf-creation-table.component.html',
  styleUrls: ['./imf-creation-table.component.scss'],
})
export class UrlEncodedImfCreationTableComponent implements OnInit, OnDestroy {
    ...
    this.subscription = this.subscribeService
      ...
      .subscribe(res => {
        if (
          res &&
          res.name &&
          res.name !== 'incomingPackager' &&
          res.name !== 'outgoingPackager' &&
          res.type !== ''
        ) {
          this.resetIMFObj();
          setTimeout(() => {
  ...
  public submitIMF(value): void {
    ...
    if (this.imfObj.type === 'fields') {
      ...
      this.imfObj.alias = this.imfObj.fieldName;

      if(value === 'save'){
        checkText = this.checkAlphaWithUndernd(this.imfObj.fieldName)
    }
    this.imfObj.fieldName = this.imfObj.fieldName.replace('.', '&#46;');
    }
    if(checkText){
    const fieldValues = {

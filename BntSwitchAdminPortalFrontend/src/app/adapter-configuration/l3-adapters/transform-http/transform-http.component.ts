...
@Component({
  selector: 'app-transform-http',
  templateUrl: './transform-http.component.html',
  styleUrls: ['./transform-http.component.scss'],
})
export class TransformHttpComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (!changes.tabIndex.firstChange && changes.tabIndex.currentValue === 2) {
      ...
      if (this.adapterData?.transformData.apiFieldsData) {
        ...
        if (networkService && networkService.value) {
          this.networkService = networkService.value;
          console.log('this.networkService', this.networkService);
        }
      }
    }
  }
  ...
  public showTransactionCondModal() {
    ...
    this.jsonTransactions.forEach(itemmap => {
  ...
  public addTransaction() {
    if (this.selectedTransactionType) {
      ...
      if (this.selectedTransactionType.length > 3) {
        ...
        this.ruleIcon = true;
        const data = this.transactionTypeValueList.find(x => x.name === messageType);
        if (!data) {
          ...
          this.transactionSelection({ value: this.selectedTransactionType });
        } else {
  ...
  private stringifyObj() {
    ...
    this.jsonTransactions = this.jsonTransactions.map(transactions => {
      if (
        ...
        transactions.request.pre_actions = this.setPreAction(transactions);
